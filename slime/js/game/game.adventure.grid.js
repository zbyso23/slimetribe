Game.Adventure.Grid = {
	...
	for( var x = 0; x < gameData.adventure.plane.gridWidth; x++ ) {
	    if( first === true ) {
		gameData.adventure.plane.grid[x] = [];
		heroes.left.grid[x] = [];
	    ...
	    for( var y = 0; y < gameData.adventure.plane.gridHeight; y++ ) {
		if( first !== true && monstersModels[x][y] !== 0 ) scene.remove( monstersModels[x][y].root );
		gameData.adventure.plane.grid[x][y] = 0;
	...
	var currX = gameData.adventure.plane.halfStep;
	var currY = 0 - gameData.adventure.plane.halfStep;
	currY += gameData.adventure.plane.step;
	var initX = gameData.adventure.plane.halfStep;
	for( x = gameData.adventure.plane.gridWidth - 1; x >= 0;  x-- ) {
	    for( y = 0; y < gameData.adventure.plane.gridHeight; y++ ) {
		Game.Scene.Graphics.addGridCube( { size: BATTLE_GRID.size, x: currX, y: 0, z: currY, gridX: x, gridY: y } );
		currX += gameData.adventure.plane.step;
	    }
	    currX = initX;
	    currY += gameData.adventure.plane.step;
	}
	...
	gameData.adventure.selection.hero.grid[params.x2][params.y2] = gameData.adventure.selection.hero.grid[params.x][params.y];
	gameData.adventure.selection.hero.monsters[params.x2][params.y2] = gameData.adventure.selection.hero.monsters[params.x][params.y];
	gameData.adventure.plane.grid[params.x2][params.y2] = gameData.adventure.plane.grid[params.x][params.y];
	monstersModels[params.x2][params.y2] = monstersModels[params.x][params.y];
	gameData.adventure.selection.hero.grid[params.x][params.y] = 0;
	gameData.adventure.selection.hero.monsters[params.x][params.y] = 0;
	gameData.adventure.plane.grid[params.x][params.y] = 0;
	...
	for( i in gridModels ) {
		...
		if( gameData.adventure.selection.hero.monsters[cords[0]][cords[1]] !== 0 ) {
		    if( gameData.adventure.selection.hero.monsters[cords[0]][cords[1]].speedRemain > 0 ) {
			Game.Scene.Graphics.setGridCube( gridModels[i].object, ADVENTURE_GRID.freeClear );
		    } else {
			Game.Scene.Graphics.setGridCube( gridModels[i].object, ADVENTURE_GRID.noMoveClear );
		    }
		} else {
	...
	if( typeof gameData.adventure.selection.hero.monsters[pos.x] === "undefined" ) return;
	var params = ADVENTURE_GRID.neutral;
	if( typeof gameData.adventure.selection.hero.monsters[pos.x][pos.y] === "object" ) {
	    params = ( gameData.adventure.selection.hero.monsters[pos.x][pos.y].speedRemain === 0 ) ? ADVENTURE_GRID.noMove : ADVENTURE_GRID.free;
	} else if( gameData.adventure.selection.enemyHero.grid[pos.x][pos.y] !== 0 ) {
	    params = ADVENTURE_GRID.enemy;
	}
	Game.Adventure.Grid.showGrid( params, pos );
    },
	...
	for( i in character.army ) {
		...
		var cords = Game.Adventure.Grid.gridToCords( tempX, monster[1] );
		Game.Adventure.Grid.addCharacter( monstersList[monsterPack.name], cords[1], 0, cords[0], tempX, monster[1], rot, left );
	    }
	}
    },
    casting: function() {
	try {
	    ...
	    var cords = Game.Adventure.Grid.findById( intersects[0].object.id );
    ...
    select: function( params ) {
        ...
        Game.Adventure.Grid.showGrid( BATTLE_GRID.free, { x: params.x, y: params.y } );
        var posAbsolute = Game.Adventure.Grid.cordsToGrid( params.x, params.y );
    ...
    unselect: function( params ) {
        ...
        Game.Adventure.Grid.showGrid( BATTLE_GRID.none, { x: params.x, y: params.y } );
